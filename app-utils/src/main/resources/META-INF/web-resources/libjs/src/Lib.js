...
//			args.push(arguments.callee.caller);
	...
	notifySuccess : function(title, message, keepTime) {
		var tpl = new Ext.XTemplate('<div class="alert alert-success alert-block alert-popup fade"><button type="button" class="close" data-dismiss="alert">&times;</button><tpl if="title"><h4>{title}</h4></tpl>{message}</div>');
...
//		}
		...
		Ext.Ajax.request({
			...
			failure : function(res, opt) {
				Ext.Msg.alert("Failure", "导出失败,可能是网络问题。");
