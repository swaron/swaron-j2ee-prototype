    ...
    renderTpl: [
        ...
        // when panel is rendered in a table-based layout.
	    	'<div id="{id}-header" class="x-boots-panel-header panel-heading {headerCls}" <tpl if="collapsible">data-toggle="collapse" data-target="#{id}-body" style="cursor:pointer;" </tpl> >',
	    		'<div class="row"><div id="{id}-headerCt"  class="col-10">',
				'{%this.renderHeader(out,values);%}',
	    		'</div>',
				'<tpl if="collapsible">', 
					'<div class="col-2"><a data-toggle="collapse" href="#{id}-body" class="pull-right x-boots-tool"><i class="icon-tool-collapse">&nbsp;</i></a></div>',
				'</tpl>',
	    		'</div>',
			'</div>',
	        '<div id="{id}-body" class="x-boots-panel-body {collapsedCls}">',
