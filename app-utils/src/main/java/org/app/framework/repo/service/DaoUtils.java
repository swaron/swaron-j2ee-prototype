...
import org.app.framework.paging.OrFilter;
...
public class DaoUtils {
	...
	public <T> List<Predicate> buildFilters(Root<T> root, List<Filter> list) {
		...
		for (Filter e : list) {
			try {
				...
				String matchStyle = e.getMatchStyle();
				...
				if (value == null || (value instanceof String && ((String)value).isEmpty() )) {
					if (Filter.EXACT_MATCH.equals(matchStyle)) {
						// 精确匹配,处理null值
				...
				if (ClassUtils.isAssignable(javaType, String.class, true)) {
					addStringPredicate(predicates, builder, property, operator, value, matchStyle);
	...
	private <T> void addStringPredicate(List<Predicate> predicates, CriteriaBuilder builder, Path<T> property,
			String operator, Object value, String matchStyle) {
		...
		if (Filter.ANY_MATCH.equals(matchStyle)) {
			stringValue = "%" + stringValue + "%";
			predicate = builder.like(builder.lower(stringProperty), stringValue);
		} else if (Filter.EXACT_MATCH.equals(matchStyle)) {
			predicate = builder.equal(stringProperty, stringValue);
		} else if (Filter.START_MATCH.equals(matchStyle)) {
			stringValue = stringValue + "%";
			predicate = builder.like(builder.lower(stringProperty), stringValue);
		} else {
			stringValue = stringValue + "%";
